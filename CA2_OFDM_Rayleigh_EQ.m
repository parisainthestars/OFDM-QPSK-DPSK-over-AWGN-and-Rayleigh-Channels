...
for ii = 1:numel(cfg.snrRange_dB)
    snrDb = cfg.snrRange_dB(ii);
    bitsHat = rayleigh_ofdm_rx_with_mmse_equalizer(tx, cfg, meta, snrDb);
...
function bitsHat = rayleigh_ofdm_rx_with_mmse_equalizer(tx, cfg, meta, snrDb)

    snrLin = 10^(snrDb/10);
    noiseVarTime = meta.signalPower / snrLin;
    noiseVarFreq = cfg.Nfft * noiseVarTime;

    rx = complex(tx);
    HposAll = cell(meta.numFrames, 1);

    for k = 1:meta.numFrames
        idx = meta.frameStarts(k) : meta.frameStarts(k) + meta.frameLen - 1;
        frameTx = tx(idx);

        xcp = reshape(frameTx, cfg.Nfft + cfg.cpLen, cfg.sfc + 1);
        x = xcp(cfg.cpLen+1:end, :);
        X = fft(x, cfg.Nfft, 1);

        H = (randn(cfg.Nfft, cfg.sfc + 1) + 1j*randn(cfg.Nfft, cfg.sfc + 1)) / sqrt(2);

        Y = H .* X;
        y = ifft(Y, cfg.Nfft, 1);
        ycp = [y(end-cfg.cpLen+1:end, :); y];

        rx(idx) = ycp(:);
        HposAll{k} = H(2:cfg.nc+1, :);
    end

    noise = sqrt(noiseVarTime/2) * (randn(size(rx)) + 1j*randn(size(rx)));
    rx = rx + noise;

    bitsHat = ofdm_rx_rayleigh_mmse(rx, cfg, meta, HposAll, noiseVarFreq);
end

function bitsHat = ofdm_rx_rayleigh_mmse(rx, cfg, meta, HposAll, noiseVarFreq)
    ...
    for k = 1:meta.numFrames
        ...
        symHat = decode_one_ofdm_frame_mmse_eq(frameRx, cfg, HposAll{k}, noiseVarFreq);
        symHatAll(outPtr:outPtr+numel(symHat)-1) = symHat;
        outPtr = outPtr + numel(symHat);
    end
...
function symHat = decode_one_ofdm_frame_mmse_eq(frameRx, cfg, Hpos, noiseVarFreq)
    ycp = reshape(frameRx, cfg.Nfft + cfg.cpLen, cfg.sfc + 1);
    y = ycp(cfg.cpLen+1:end, :);
    Y = fft(y, cfg.Nfft, 1);
    ...
    XhatPos = Ypos .* conj(Hpos) ./ (abs(Hpos).^2 + noiseVarFreq + eps);

    Dhat = XhatPos.'; % (sfc+1) x nc
